...
// Modified by: Ho Tin Hung
// Add partial write tailored to resolve coherence/meta overwrite
...
module tc_sram_meta #(
  ...
  parameter int unsigned NumBitsCacheMeta = 32,
  ...
  input  logic  [NumPorts-1:0] is_cache_meta_i, // is cache meta data access
  // output ports
            ...
            if (we_i[i]) begin
              ...
              for (int unsigned j = 0; j < BeWidth; j++) begin
                if (be_i[i][j]) begin
                  ...
                  if (is_cache_meta_i[i]) begin
                    sram[addr_i[i]][NumBitsCacheMeta-1:0] <= wdata_i[i][NumBitsCacheMeta-1:0];
                  end else begin
                    sram[addr_i[i]][j*ByteWidth+:ByteWidth] <= wdata_i[i][j*ByteWidth+:ByteWidth];
                  end
                end
